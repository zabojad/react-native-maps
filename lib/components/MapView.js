...
const propTypes = {
   ...
   * Url KML Source
   */
  kmlSrc: PropTypes.array,
  
  /**
   * Current KML Source index
   */
  kmlLayerIndex: PropTypes.number,
...
class MapView extends React.Component {
  ...
  didKmlTapAt(location) {
    if (Platform.OS === 'android') {
      return NativeModules.AirMapModule.didKmlTapAt(this._getHandle(),location);
    } else if (Platform.OS === 'ios') {
      return this._runCommand('didKmlTapAt', [location]);
    }
    return Promise.reject('didKmlTapAt not supported on this platform');
  }
